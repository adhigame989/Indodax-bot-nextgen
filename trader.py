    ...
    def scan_phase(self):
        for snap in self.scanner.scan():
            try:
                ...
                amount_idr = config.BASE_TRADE_AMOUNT

                if hasattr(self.buyer, "buy"):
                    self.buyer.buy(snap["symbol"], amount_idr, ask)
                else:
                    amount = amount_idr / ask
                    self.buyer.execute(snap["symbol"], amount)

            except Exception as exc:
                error(f"SCAN {snap.get('symbol','?')}: {exc}")
        ...
        for symbol, layers in self.positions.all().items():
            try:
                ...
                for layer in list(layers):
                    ...
                    if hasattr(self.positions, "update_market") and layer_id:
                        self.positions.update_market(symbol, layer_id, price)
                    ...
                    if hasattr(self.seller, "sell") and layer_id:
                        self.seller.sell(symbol, layer_id, price, reason)
                    else:
